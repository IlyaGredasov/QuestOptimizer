...
from graph import QuestLine, Path, PathState, Graph
...
class QuestOptimizer:
    def floyd(self) -> list[list[Path]]:
        ...
        ways = [[Path(path=[], length=float("inf")) for _ in range(len(self.graph))] for _ in range(len(self.graph))]
        for i1 in range(len(path_matrix)):
            for i2 in range(len(path_matrix)):
                if path_matrix[i1][i2] != float("inf"):
    ...
    def __init__(self,
        ...
        self.best_path: Path = Path([], float("inf"))
    ...
    def optimize_step(self) -> None:
        ...
        if current_state.count_of_quests() <= max(self.minimum_quest_count, 1) * self.error_afford:
            logging.info(
                f"{self.minimum_quest_count}, {len(self.queue)}")
            ...
            if len(current_state.quests) == 0:
                ...
                with self.lock:
                    for j in range(len(self.graph)):
                        if self.graph[current_state.current_node_index][j] != float("inf"):
                            new_state: PathState = deepcopy(current_state)
    ...
    def optimize(self) -> None:
        ...
        threads = []
        logging.info("Queue has been loaded")
        for i in range(self.num_threads):
            thread = threading.Thread(target=self.optimize_cycle, name=f"Thread-{i}")
    ...
    def print_quests_on_path(self):
        ...
        for node_index in self.best_path:
            print(node_index, self.vertex_indexes_to_names[node_index], self.vertex_indexes_to_names[node_index],
                  end=' ')
